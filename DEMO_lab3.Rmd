---
title: "Lab 3: Topic Analysis DEMO"
author: "Erica Bishop"
date: "2023-04-19"
...
```{r corpus}
corp_sb <- corpus(x = tbl, text_field = "text") # make the corpus
stories_stats <- summary(corp_sb) # grab some stats
stories_stats
```
...
toks <- tokens(corp_sb, remove_punct = TRUE, remove_numbers = TRUE)

#add some of the common low value words from lab 2
add_stops <- c(stopwords("en"), "spring", "break", "Spring", "Break") # grab stopwords and spring break
...
```{r LDA_modeling}
k <- 3 # three main topics

# Estimate LDA model -- shuffles words into topics by frequency and rates of co-occurance
...
dim(beta)
dim(theta)
terms(topicModel_k3, 10) # see top ten terms in each topic
```
...
topic_words <- terms(topicModel_k10, 5) #take five words from each topic to names
...
# colnames(example_props) <- topic_names #this is not working for me
...
For this assignment you'll the articles data you downloaded from Nexis Uni in Week 2. (note to self: ok to switch up search terms and filtering to maybe get better body of articles)
